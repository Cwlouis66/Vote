...
//imports the ABI of a smart contract and to interact with it 
const {abi} = require('./artifacts/contracts/Voting.sol/Voting.json');

//connects to the Ethereum network
const provider = new ethers.providers.JsonRpcProvider(API_URL);
//connects to the Ethereum network and signs the transactions
...
app.post("/vote", async (req, res) => {
    ...
    async function storeDataInBlockchain(vote) {
        ...
        const transactions = await contractInstance.addCandidate(vote);
        await transactions.wait();
    }
    //checks if the voting is still open
