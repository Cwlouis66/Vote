...
let CONTRACT_ADDRESS = '0x0a2884BBc7C5CaaF49d9acDEe46C37D2860cF34e';
let contractAbi = [
    {
      "inputs": [
        {
  ...
  const connectMetamask = async () => {
    ...
    element.innerHTML = "Metamask is connected to: " + WALLET_CONNECTED;
  };
  ...
  const getAllCandidates = async () => {
    ...
    p3.innerHTML = "Candidates are all up to date";
  };
  ...
  const voteStatus = async () => {
    if (WALLET_CONNECTED != 0) {
      ...
      const signer = provider.getSigner();
      const contractInstance = new ethers.Contract(CONTRACT_ADDRESS, contractAbi, signer);
      const curStatus = await contractInstance.getVotingStatus();
      const time = await contractInstance.getRemainingTime();
      status.innerHTML = curStatus == 1 ? "Voting is open" : "Voting has finished";
